...
export default function NotFound() {
  ...
  useEffect(() => {
    const svg = d3.select("#monkey_404");
    const tail = svg.select("#tail");

    // دالة لإرجاع حركة الذيل إلى وضعها الأصلي
    const revert = function () {
      tail.transition()
        .duration(1600)
        .ease(d3.easeSinInOut)
        .attr("d", "M89,315c2.2-15.2-23-13.2-21.6,4.8c1.7,22.3,24.4,22.1,42.5,9.1c10.8-7.8,15.3-1.8,19.1,1.1c2.3,1.7,6.7,3.3,11-3");
    };

    // تشغيل حركة الذيل بشكل دوري
    const intervalId = setInterval(() => {
      tail.transition()
        .duration(1600)
        .ease(d3.easeSinInOut)
        .attr("d", "M81,310c-8.8-6.5-20.8,6.5-15,18c7.4,14.5,22.5,10.8,31,3c9.8-9,18.9-5.6,22-2c5.8,6.8,16.7,4.3,21-2")
        .on("end", revert); // التأكد من حدوث التراجع بعد نهاية الحركة
    }, 3200);
...
<NavLink
      to="#"  // You can set this to "#" or leave it empty since we're handling the click with navigate
      onClick={handleBackClick}
      className="m-auto nav-link px-4 py-2 rounded-1 fs-5 mt-2"
      style={{ backgroundColor: "#89949b", color: "white", width: "fit-content" }}
    >
      Back
    </NavLink>
  </div>
